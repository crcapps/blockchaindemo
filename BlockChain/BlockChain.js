...
require('../object') // polyfill Object.fromEntries() for node < 12.0.0
  ...
  return {
    ...
    allTransactions() {
      const transactions = this.chain.map(block => block.transactions)
        ...
        }) // I didn't want to polyfill Array.prototype.flat() for node < 11.0.0
    ...
    */
    ...
    owner(index = 0, shouldTrace = false) {
      return shouldTrace ? this.transactionsForIndex(index)
        .sort(byTimestamp)
        .reduce((_, current) => {
          return current.recipient
        }, '') : this.transactionsForIndex(index)
          .sort(byTimestamp)
          .slice(-1)[0].recipient
    },
